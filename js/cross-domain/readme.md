...
- 前后端联调 
    ...
    - 同源
        ...
        协议不同也不同源，为什么这么严格？
        不同协议之间安全等级不同，例如http，是明文传输，https 是加密传输，明文传输的网站怎能随意访问密文传输的网站资源呢？
        http://localhost:5173 -> https://localhost:5173/api/user
        cross domain（origin） 跨域，跨的是 origin，而不是简单的跨域名
...
- 为什么要学习跨域？
    - 前后端分离式日常开发的形式，端口或域名不一样的情况肯定会进程碰到
    - CORS(Cross-Origin Resource Sharing) Policy 同源策略？
        浏览器端的机制
    - 跨域请求被block掉了
        请求到达了服务器端吗？
        到达了
    ...
    - 安全问题
        ...
        - 后端的安全
    - 怎么解决跨域？
        - 既拿到cross origin 的资源，又不违反CORS机制
